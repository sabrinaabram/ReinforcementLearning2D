...
for r=1:repeats
    ...
    for s=1:steps
        ...
        elseif s == tHold
            % initial parameter guess
%             bEst = bActual.*rand; % randn(length(bActual),1); %bActual; % random guess  
            R = 1000.*eye(length(bEst)); % This is used in RLS and it will converge over time as it learns. 
            % learning rate for "policy" gradient descent
            alpha = 2;
        end
        ...
        aNew = a + exec_noise*randn(1,length(a)); % a new action centered about the estimated optimal value
        aSave(s,:) = aNew;
        
        % get temporary reward
        rewardNew = bActual(1)*aNew(1) + bActual(2)*aNew(2) + bActual(3)*(aNew(1))^2 + ...
               bActual(4)*(aNew(1))*(aNew(2)) + bActual(5)*(aNew(2))^2 + meas_noise.*randn;
        
        % gradient descent learning value function parameters
        % this is the gradient wrt the action
        % the gradient is the reward of the previous action with previous
        % function parameters to the reward of the current action with
        % current function parameters
        if s > 1
%             gradient(s,:) = costLandscapesDerivative(action(s-1,:),bEst)';
            
            gradient = (rewardNew - reward(s-1))./(aNew - action(s-1,:));
            % s^(-alpha) for the learning rate to decay over time and eliminate oscillations
            action(s,:) = action(s-1,:) - alpha.*sign(action(s-1,:)).*gradient;
            if s < tHold
                action(s,2) = aNew(2); % frequency hold with variability
            end
        else
            action(s,:) = aNew;
        end
        ...
        reward(s) = bActual(1)*action(s,1) + bActual(2)*action(s,2) + bActual(3)*(action(s,1))^2 + ...
               bActual(4)*(action(s,1))*(action(s,2)) + bActual(5)*(action(s,2))^2 + meas_noise.*randn;
        ...
        k = 1;
        x = [action(s,1) action(s,2) (action(s,1))^2 ...
            (action(s,1))*(action(s,2)) (action(s,2))^2]';
        ...
        reward_all(s,:) = reward(s);
        action_all(s,:) = action(s,:);
