...
param = 0.1;
K_ss_hold = kernel(Stest_hold,Stest_hold,param); % initial covariance matrix for speed

% Cholesky decomposition (square root) of the covariance matrix
L_hold = chol(K_ss_hold + 1e-15*eye(length(K_ss_hold)),'lower');
...
% I think this is the spread or variance for each point to be used in the
% radial basis calculation
K_ss_sp = kernel(Stest,Stest,param); % initial covariance matrix for speed
K_ss_freqsp = kernel(Ftest,Stest,param); % initial covariance matrix for frequency, speed
K_ss_spfreq = kernel(Stest,Ftest,param); % initial covariance matrix for speed, frequency
K_ss_freq = kernel(Ftest,Ftest,param); % initial covariance matrix for frequency

K_ss = [K_ss_sp K_ss_spfreq;...
        K_ss_freqsp K_ss_freq];

% Cholesky decomposition (square root) of the covariance matrix
L = chol(K_ss + 1e-15*eye(length(K_ss)),'lower');
...
for r=1:repeats
    ...
    for s=1:steps
        ...
        if n == 1
            ...
            K = kernel(Strain_x, Strain_x, param);
            L = chol(K + 0.00005*eye(length(Strain_x)),'lower');

            % Compute the mean at our test points
            K_s = kernel(Strain_x, Stest_hold, param);
            Lk = mldivide(L,K_s);
            mu = Lk'*mldivide(L,train_y);

            % Draw samples from the posterior at our test points.
            L = chol(K_ss_hold + 1e-6*eye(length(K_ss_hold)) - Lk'*Lk);
            f_post = mu + L*normrnd(0,1,[length(Stest_hold),1]);
            ...
            % Apply the kernel function to our training points
            K_sp = kernel(Strain_x,Strain_x,param);
            K_freqsp = kernel(Ftrain_x,Strain_x,param);
            K_spfreq = kernel(Strain_x,Ftrain_x,param);
            K_freq = kernel(Ftrain_x,Ftrain_x,param);

            K = [K_sp K_spfreq;...
                 K_freqsp K_freq];
            
            L = chol(K + 0.0005*eye(length(Strain_x)),'lower');

            % Compute the mean at our test points
            K_s_sp = kernel(Strain_x,Stest,param); % initial covariance matrix for speed
            K_s_freq = kernel(Ftrain_x,Ftest,param); % initial covariance matrix for frequency
            
            K_s = [K_s_sp; K_s_freq];
            
            Lk = mldivide(L,K_s);
            for i = 1:n
                mu(:,n) = Lk(n,:)'*mldivide(L,[train_y;train_y]);
            end

            % Draw samples from the posterior at our test points.
            L = chol(K_ss + 1e-6*eye(length(K_ss)) - Lk(:)'*Lk(:));
            f_post = mu_S + L_S*normrnd(0,1,[length(Stest),1]);
            
            % actions
            % Qa_est contains every possible combination of actions
            [~,actionIndx] = min(f_post);
