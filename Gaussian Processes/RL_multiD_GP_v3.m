...
%             Qa_est = E(speedHoldi,:);
        ...
        elseif s == tHold
            ...
            Qa_est = 100.*ones(size(E));
            Qa_est(speedHoldi,:) = E(speedHoldi,:);
            Qa_est = Qa_est(:);            
%             Qa_est = E(:);
        ...
        if n == 1
            ...
            [aOpt{:}]=ind2sub(size(E),minActual);
            aOpt = cell2mat(aOpt);
            OptimalandHold(s,:) = [Frequency(aOpt(1)) Speed(aOpt(2))];
            
            % deal with edges
            for i = 1:n
                if actionIndx(i) < 1
                    actionIndx(i) = 1;
                elseif actionIndx(i) > maxlength
                    actionIndx(i) = maxlength;
                end
            end
            
            % convert back to single index for Q
            actionIndx=num2cell(actionIndx);
            actionIndx = sub2ind(size(E),actionIndx{:});
        end
        
        % clear variables
        if s == tHold-1 || s == steps
